...
# 10. Staj İçin En Önemli Kısımlar

Staj hazırlığında en kritik Python konuları:
...
list comprehension
import
pandas'a geçiş mantığı
```

Çünkü pandas’a geçtiğinde aslında şunu yapacaksın:
...
Ama pandas DataFrame mantığını anlamak için önce list/dict yapısını iyi bilmek gerekir.

Özellikle şu yapı çok önemli:
...
Bu yapı pandas DataFrame’e çok benzer:
...
Bugünkü `pandas-analysis/python_basics.py` dosyan için önerilen pratik:
...
sales = [
    {"product": "Laptop", "category": "Electronics", "price": 35000, "quantity": 2},
    {"product": "Mouse", "category": "Accessories", "price": 750, "quantity": 5},
    {"product": "Keyboard", "category": "Accessories", "price": 1500, "quantity": 3},
    {"product": "Monitor", "category": "Electronics", "price": 8000, "quantity": 1}
]

for sale in sales:
    total = calculate_total(sale["price"], sale["quantity"])
    print(f"{sale['product']} total sales: {total} TL")

expensive_sales = [sale for sale in sales if sale["price"] > 1000]
print("Expensive sales:", expensive_sales)
```
...
```text
notes/kaggle_python_basics_summary.md
```

Commit mesajı:

```bash
git add notes/kaggle_python_basics_summary.md
git commit -m "add Kaggle Python basics summary notes"
git push
```
